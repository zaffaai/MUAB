...
export default function Sidebar() {
  ...
  return (
    <aside
      className={`fixed left-0 top-0 h-screen bg-(--background-secondary) border-r border-(--border-color) transition-all duration-300 z-40 ${
            ...
            >
              ...
              {!isCollapsed && (
                <>
                  ...
                  {link.badge && (
                    <span className="text-xs px-2 py-0.5 rounded-full bg-linear-to-r from-purple-500 to-pink-500 text-white">
                      {link.badge}
                    </span>
                  )}
                </>
          ...
          {navGroups.map((group) => (
            <div key={group.label} className="mt-2">
              <button
                ...
                className="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-(--text-secondary) hover:bg-(--background-tertiary) transition-all"
              >
              ...
              {!isCollapsed && expandedGroups.includes(group.label) && (
                <div className="ml-4 mt-1 space-y-1 border-l-2 border-(--border-color) pl-4">
                  {group.sublinks.map((sublink) => (
                    <Link
                      ...
                      className={`flex items-center gap-2 px-3 py-2 rounded-lg text-sm transition-all ${
                        isActive(sublink.href)
                          ? 'bg-(--color-primary-500) bg-opacity-10 text-primary font-medium'
                    ...
                    >
                      ...
                      {sublink.count && (
                        <span className="text-xs px-1.5 py-0.5 rounded-full bg-(--background-tertiary) text-(--text-tertiary)">
                          {sublink.count}
                        </span>
                      )}
                    </Link>
